...
public class Rebuld {
    ...
    public TreeNode buildTree2(int[] preorder, int[] inorder) {
        ...
        return recursive2(0,0,inorder.length-1);
    }
    public TreeNode recursive2(int preRootIndex,int inleft,int inright){
        if(inleft>inright){
            return null;
        }
        TreeNode root = new TreeNode(preorder[preRootIndex]);
        int idx = map.get(preorder[preRootIndex]);
        root.left= recursive(preRootIndex + 1,inleft,idx-1);
                root.left=recursive(preRootIndex + 1,idx+1,inright);
